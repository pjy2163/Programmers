### 요약
문제 : a, b 둘 중에 한명만 물건 훔치기. 물건은 총 i개 존재
a는 n이하, b는 m이하여야 함. a의 n 개수 최소화 시키기

-> dp : 누적 합 계산하면서 이미 계산된 경로는 다시 계산하지 않도록.

초기화:
  dp[0][0][0] = true //아무 것도 훔치지 않은 상태

반복 (모든 물건 i에 대해서):
  각 가능한 흔적 상태 (a, b)에 대해:
    - A가 훔친다면:
        dp[i+1][a + info[i][0]][b] = true
    - B가 훔친다면:
        dp[i+1][a][b + info[i][1]] = true

결과 찾기:
  dp[itemCount][a][b] == true인 상태 중
    A의 흔적 a가 가장 작은 값 리턴
  없으면 -1
